...
struct Material {
    ...
    Material(vec3 _ka, vec3 _kd, vec3 _ks, float _shininess) : ka(_ka), kd(_kd), ks(_ks) { shininess = _shininess; rough = true; }
...
struct Ray {
    vec3 start, dir;
    bool out;
    Ray(vec3 _start, vec3 _dir, bool a = false) { start = _start; dir = _dir.normalize(); out = a; }
};
...
struct Paraboloid : public Intersectable {
    ...
    Hit intersect(const Ray& ray) {
        ...
        hit.position = ray.start + ray.dir * hit.t;
        if (hit.position.z > height + start.z) return ignore;
...
struct Cone : public Intersectable {
    ...
    Hit intersect(const Ray& ray) {
        ...
        hit.position = ray.start + ray.dir * hit.t;
        if (hit.position.z > height || hit.position.z < 0) return ignore;
        hit.normal.x = (hit.position.x - start.x) / (radius*radius);
        hit.normal.y = (hit.position.y - start.y) / (radius*radius);
...
struct FiniteVerticalPlane : public Intersectable {
    ...
    Hit intersect(const Ray& ray) {
        ...
        hit.normal = normal;
        if (dot(hit.normal, ray.dir) > 0) hit.normal = hit.normal * (-1);
        if (sqrtf((hit.position - start).x*(hit.position - start).x + (hit.position - start).y*(hit.position - start).y) > width || sqrtf((hit.position - start).z*(hit.position - start).z) > height) return ignore;
        hit.material = material;
        return hit;
    }
};
...
struct TexturedFiniteVerticalPlane : public Intersectable {
    ...
    Hit intersect(const Ray& ray) {
        ...
        if (sqrtf((hit.position - start).x*(hit.position - start).x + (hit.position - start).y*(hit.position - start).y) > width || sqrtf((hit.position - start).z*(hit.position - start).z) > height) return ignore;
        hit.material = material;
        if ((fabs(fmod(hit.position.x / 0.25, 2.0)) >= 1.0 || fabs(fmod(hit.position.y / 0.25, 2.0)) >= 1.0) && (fabs(fmod(hit.position.z / 0.25, 2.0)) <= 1.0)) hit.material = mat2;
        return hit;
    }
};
...
struct FiniteHorizontalPlane : public Intersectable {
    ...
    Hit intersect(const Ray& ray) {
        ...
        hit.normal = normal;
        if (dot(hit.normal, ray.dir) > 0) hit.normal = hit.normal * (-1);
        if (sqrtf((hit.position - start).x*(hit.position - start).x) > width || sqrtf((hit.position - start).y*(hit.position - start).y) > height) return ignore;
        hit.material = material;
        return hit;
    }
};
...
struct TexturedFiniteHorizontalPlane : public Intersectable {
    ...
    Hit intersect(const Ray& ray) {
        ...
        if (sqrtf((hit.position - start).x*(hit.position - start).x) > width || sqrtf((hit.position - start).y*(hit.position - start).y) > height) return ignore;
        hit.material = material;
        if ((fabs(fmod(hit.position.x / 0.25f, 2.0f)) >= 1.0f) && (fabs(fmod(hit.position.y / 0.25f, 2.0f)) <= 1.0f)) hit.material = mat2;
        return hit;
    }
};
